...
if selected_page == "Distribusi Penggunaan Obat per Provider":
    ...
    st.dataframe(df)
# Pilihan untuk setiap filter berdasarkan data yang sudah difilter
item_options = filtered_df['Nama Item Garda Medika'].dropna().unique() if 'Nama Item Garda Medika' in filtered_df else []
golongan_options = filtered_df['Golongan'].dropna().unique() if 'Golongan' in filtered_df else []
subgolongan_options = filtered_df['Subgolongan'].dropna().unique() if 'Subgolongan' in filtered_df else []
komposisi_options = filtered_df['Komposisi Zat Aktif'].dropna().unique() if 'Komposisi Zat Aktif' in filtered_df else []
